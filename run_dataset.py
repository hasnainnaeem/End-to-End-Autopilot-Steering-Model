...
@Gooey(required_cols=2,
       program_name="Run on Dataset - End to End Self Driving",
       timing_options={
           'show_time_remaining': False,
       }
       )
def parse_args():
    prog_description = 'Test driving model on a dataset. ' \
                       'Dataset should contain the frames of video ' \
                       'for which steering angle is to be predicted.'
    ...
    range_parser.add_argument('--dataset-dir',
                              default=str(os.path.join(os.getcwd(), "driving_dataset", "images")),
                              help="Directory containing dataset frames",
                              widget="DirChooser")

    return parser.parse_args()


def run_dataset(dataset_dir_url):
    ...
    while cv2.waitKey(10) != ord('q'):
        full_image = cv2.imread(os.path.join(dataset_dir_url, str(i) + ".jpg"))
